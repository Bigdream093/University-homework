...
test('upload retries query persisted results before resending; payload is not kept as plaintext', async () => {
  ...
  api.request = async (config) => {
    ...
    assert.equal(transfer.state.value, '正在保存，请勿重复提交')
    serverSaved = true
    throw new Error('response lost')
  }
  api.get = async () => {
    queries++
    if (queries === 1) throw new Error('network unavailable')
    return { data: { state: 'succeeded', result: { id: 1, receipt_no: 'test-receipt' } } }
  }
  try {
    const args = {
      url: '/assignments/1/submit',
      statusUrl: '/assignments/1/upload-status/',
      fields: { content: 'sensitive answer' },
    }
    await assert.rejects(transfer.run(args), /response lost/)
    assert.equal(serverSaved, true)
    assert.ok(!sessionStorage.values().join().includes('sensitive answer'))
    const result = await transfer.run(args)
    assert.equal(result.receipt_no, 'test-receipt')
...
test('cancelled wait queries completion; current upload is protected from double click', async () => {
  ...
  api.request = async (config) =>
    new Promise((resolve, reject) => {
      ...
      config.signal.addEventListener('abort', release)
    })
  ...
  try {
    const args = {
      url: '/materials/1',
      method: 'put',
      statusUrl: '/materials/1/upload-status/',
      fields: { title: '资料' },
    }
    const pending = transfer.run(args)
    await assert.rejects(transfer.run(args), /尚未结束/)
    transfer.cancel()
    const result = await pending
    assert.equal(result.receipt_no, 'already-saved')
