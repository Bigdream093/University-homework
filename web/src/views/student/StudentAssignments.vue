<script setup>
import { contentSummary } from '../../utils/richText.js'
...
async function refreshUnread() {
  ...
  try {
    ...
    if (sequence === unreadSequence)
      unread.value = data.find((course) => course.id === courseId)?.unread_notice_count || 0
  } catch {}
}
useRefresh(() => {
  load()
  ...
  <div>
    <div class="page-head">
      <div>
        ...
        <h1>
          {{ course.name || '课程作业' }} {{ course.status === 'archived' ? '（已归档）' : '' }}
        </h1>
    ...
    <div ref="tabsRoot">
      <el-tabs v-model="tab" class="section-tabs">
        <el-tab-pane
          ...
          :key="def.name"
          :label="def.name === 'notices' && unread ? def.label + '（' + unread + '）' : def.label"
          :name="def.name"
          lazy
        >
          <div v-if="def.name === 'notices'" class="panel">
            <StudentNotices
              v-if="tab === 'notices'"
              :key="`notices-${route.params.id}`"
          ...
          <div v-else-if="def.name === 'assignments'" class="panel">
            ...
            >
              <div class="card-head" @click="assignmentCard.toggle(assignment.id)">
                ...
                <h3 class="card-title">
                  {{ assignment.title }}
                  <small>{{ assignment.work_mode === 'group' ? '分组作业' : '个人作业' }}</small>
                </h3>
                <span class="hint">截止：{{ assignment.effective_deadline || '不限时间' }}</span>
              ...
              <div v-if="assignmentCard.isOpen(assignment.id)" class="card-body">
                ...
                >
                  {{
                    contentSummary(assignment.description, assignment.description_format) ||
                    '查看作业详情并提交'
                  }}
                </p>
                ...
                <span
                  v-if="
                    deadline(assignment).kind === 'warning' || deadline(assignment).kind === 'late'
                  "
          ...
          <div v-else-if="def.name === 'questions'" class="panel">
            <CourseQuestions
              v-if="tab === 'questions'"
              :key="`questions-${route.params.id}`"
              :course-id="route.params.id"
              :readonly="course.status === 'archived'"
            />
          </div>
          <div v-else-if="def.name === 'materials'" class="panel">
            <StudentMaterials :key="`materials-${route.params.id}`" :course-id="route.params.id" />
          </div>
        </el-tab-pane>
      </el-tabs>
    </div>
