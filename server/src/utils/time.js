...
export function parseTime(value) {
  ...
  return Number.isFinite(date.getTime()) && formatter.format(date).replace('T', ' ') === value
    ? date.getTime()
    : NaN
}
...
export function isLate(deadline, now = nowText()) {
  return deadline ? Number(parseTime(now) > parseTime(deadline)) : 0
}
export function isFuture(value, now = nowText()) {
  return parseTime(value) > parseTime(now)
}
