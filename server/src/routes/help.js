...
const router=Router();
function chapters(role){return index.filter(c=>c.role==='public'||c.role===role).map(c=>({...c,body:fs.readFileSync(new URL('../../help/'+c.id+'.md',import.meta.url),'utf8'),version:'2026.09'}));}
router.get('/help/public',(_req,res)=>res.json(chapters('public')));
router.get('/help',auth,(req,res)=>{const keyword=String(req.query.q||'').toLowerCase();res.json(chapters(req.user.role).filter(c=>(c.title+' '+c.body).toLowerCase().includes(keyword)));});
router.get('/help/download',auth,(req,res)=>res.attachment('mohen-'+req.user.role+'-manual.md').type('text/markdown; charset=utf-8').send(chapters(req.user.role).map(c=>c.body).join('\n\n---\n\n')));
router.get('/help/:id',auth,(req,res)=>{const c=chapters(req.user.role).find(c=>c.id===req.params.id);if(!c)return res.status(404).json({message:'没有这个帮助章节'});res.json(c);});
