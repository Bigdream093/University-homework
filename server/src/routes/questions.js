...
function visibleQuestion(id, user, write = false) {
  const question = db.prepare('SELECT * FROM course_questions WHERE id=?').get(id)
  if (!question) fail(404, '问题不存在')
  courseAccess(question.course_id, user, { write })
  if (user.role !== 'teacher' && question.student_id !== user.id) fail(404, '问题不存在')
  return question
}
...
function assertEditable(question) {
  if (
    db.prepare('SELECT 1 FROM question_replies WHERE question_id=?').get(question.id) ||
    db.prepare('SELECT 1 FROM question_publications WHERE question_id=?').get(question.id)
  )
...
router.get('/courses/:id/questions/public', auth, (req, res) => {
  ...
  const teacher = req.user.role === 'teacher',
    statusSql = teacher ? "p.status IN ('published','withdrawn')" : "p.status='published'"
  res.json(
    db
      .prepare(
        `SELECT p.id,p.question_id,p.summary,p.reply,p.status,p.created_at,p.withdrawn_at,q.sort_order,q.pinned FROM question_publications p JOIN course_questions q ON q.id=p.question_id WHERE q.course_id=? AND q.hidden=0 AND ${statusSql} AND (p.summary LIKE ? OR p.reply LIKE ?) ORDER BY q.pinned DESC,q.sort_order,q.id,p.id DESC LIMIT ? OFFSET ?`,
      )
...
router.post('/courses/:id/questions', auth, studentOnly, (req, res) => {
  const course = courseAccess(req.params.id, req.user, { write: true }),
    at = nowText()
  const order = db
    .prepare('SELECT COALESCE(MIN(sort_order),0)-1 value FROM course_questions WHERE course_id=?')
    .get(course.id).value
  const id = db
    .prepare(
      'INSERT INTO course_questions(course_id,student_id,title,content,must_private,sort_order,created_at,updated_at) VALUES(?,?,?,?,0,?,?,?)',
    )
    .run(
      course.id,
      req.user.id,
...
router.get('/questions/:id', auth, (req, res) => {
  ...
  res.json({
    ...
    replies: db
      ...
      .all(question.id),
    publications: db
      .prepare('SELECT * FROM question_publications WHERE question_id=? ORDER BY id DESC')
      .all(question.id),
...
router.put('/questions/:id', auth, studentOnly, (req, res) => {
  ...
  db.prepare('UPDATE course_questions SET title=?,content=?,updated_at=? WHERE id=?').run(
    textValue(req.body.title, '标题', 200),
...
router.post('/questions/:id/replies', auth, (req, res) => {
  ...
  db.transaction(() => {
    db.prepare(
      'INSERT INTO question_replies(question_id,author_id,content,created_at) VALUES(?,?,?,?)',
    ).run(question.id, req.user.id, textValue(req.body.content, '回复内容'), at)
    db.prepare('UPDATE course_questions SET status=?,updated_at=? WHERE id=?').run(
      req.user.role === 'teacher' ? 'answered' : 'open',
      at,
      question.id,
    )
  })()
...
router.post('/questions/:id/publish', auth, teacherOnly, (req, res) => {
  db.transaction(() => {
    ...
    db.prepare(
      'INSERT INTO question_publications(question_id,teacher_id,summary,reply,created_at) VALUES(?,?,?,?,?)',
    ).run(question.id, req.user.id, summary, reply, nowText())
    recordEvent(question, req.user, 'publish')
  })()
  res.status(201).json({ message: '已公开摘要和答复，私人原帖及后续追问不会公开' })
})
router.post('/questions/:id/withdraw', auth, teacherOnly, (req, res) => {
  db.transaction(() => {
    const question = visibleQuestion(req.params.id, req.user)
    if (
      !db
        .prepare("SELECT 1 FROM question_publications WHERE question_id=? AND status='published'")
        .get(question.id)
    )
      fail(409, '没有已公开的摘要')
    withdrawPublications(question, req.user)
...
router.delete('/questions/:id/publications/:pubId', auth, teacherOnly, (req, res) => {
  db.transaction(() => {
    const question = visibleQuestion(req.params.id, req.user, true)
    const publication = db
      .prepare('SELECT id FROM question_publications WHERE id=? AND question_id=?')
      .get(req.params.pubId, question.id)
    if (!publication) fail(404, '公开摘要不存在')
    db.prepare('DELETE FROM question_publications WHERE id=?').run(publication.id)
    recordEvent(question, req.user, 'delete_publication')
  })()
  res.json({ message: '公开摘要已删除，学生端立即消失' })
})
router.post('/questions/:id/publications/:pubId/move', auth, teacherOnly, (req, res) => {
  const question = visibleQuestion(req.params.id, req.user, true),
...
router.put('/questions/:id/manage', auth, teacherOnly, (req, res) => {
  db.transaction(() => {
    const question = visibleQuestion(req.params.id, req.user)
    if (req.body.hidden !== true || req.body.status !== undefined || req.body.pinned !== undefined)
      courseAccess(question.course_id, req.user, { write: true })
    const status = req.body.status ?? question.status
    if (!['open', 'answered', 'resolved'].includes(status)) fail(400, '无效状态')
    const hidden = req.body.hidden === undefined ? question.hidden : Number(!!req.body.hidden)
    if (hidden) withdrawPublications(question, req.user)
    db.prepare(
      'UPDATE course_questions SET status=?,pinned=?,hidden=?,updated_at=? WHERE id=?',
    ).run(
      status,
      req.body.pinned === undefined ? question.pinned : Number(!!req.body.pinned),
      hidden,
      nowText(),
      question.id,
    )
