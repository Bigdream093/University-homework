...
router.get('/courses/:id/groups', auth, (req, res) => {
  ...
  res.json(
    req.user.role === 'teacher'
      ? groups
      : groups.filter((group) => group.members.some((member) => member.id === req.user.id)),
  )
})
function saveGroup(req, id) {
  ...
  const course = courseAccess(existing?.course_id || req.params.id, req.user, {
      write: true,
      teacher: true,
    }),
    memberIds = resolveMembers(req.body.member_ids, course.id, id || 0),
    leader = Number(req.body.leader_id)
  if (!memberIds.includes(leader)) fail(400, '组长必须是本组成员')
  const name = textValue(req.body.name, '组名', 100)
  if (
    db
...
router.post('/courses/:id/groups', auth, teacherOnly, (req, res) =>
  res.status(201).json(db.transaction(() => saveGroup(req))()),
)
router.put('/groups/:id', auth, teacherOnly, (req, res) =>
  res.json(db.transaction(() => saveGroup(req, Number(req.params.id)))()),
)
router.delete('/groups/:id', auth, teacherOnly, (req, res) => {
  const group = db.prepare('SELECT * FROM course_groups WHERE id=?').get(req.params.id)
  if (!group) fail(404, '小组不存在')
  courseAccess(group.course_id, req.user, { write: true })
  db.prepare('DELETE FROM course_groups WHERE id=?').run(group.id)
  res.json({ message: '模板小组已删除，已发布作业快照不受影响' })
})
router.get('/assignments/:id/groups', auth, (req, res) => {
  ...
  res.json(
    req.user.role === 'teacher'
      ? rows
      : rows.filter((group) => group.members.some((member) => member.id === req.user.id)),
  )
})
router.post('/assignments/:id/groups/snapshot', auth, teacherOnly, (req, res) => {
  db.transaction(() => {
    ...
    if (assignment.status !== 'draft' || assignment.groups_locked || assignment.work_mode !== 'group')
      fail(400, '仅未发布的分组作业草稿可以配置快照')
    const groupIds = [...new Set((req.body.group_ids || []).map(Number))]
...
router.put('/assignment-groups/:id/submitter', auth, teacherOnly, (req, res) => {
  const group = db.prepare('SELECT * FROM assignment_groups WHERE id=?').get(req.params.id)
  if (!group) fail(404, '分组不存在')
  const assignment = assignmentAccess(group.assignment_id, req.user, { write: true }),
    submitterId = Number(req.body.submitter_id)
  if (
    !db
      .prepare(
