...
router.get('/assignments/:id/export', auth, teacherOnly, async (req, res) => {
  ...
  const rows = teacherRows(assignment).flatMap(r => assignment.work_mode==='group' ? r.members.map(m => ({...r,username:m.username,name:m.name,group_name:r.name})) : [r]);
  if(assignment.work_mode==='group'){
    const ids=new Set(db.prepare('SELECT student_id FROM assignment_group_members WHERE assignment_id=?').all(assignment.id).map(m=>m.student_id));
    for(const m of db.prepare('SELECT u.id,u.username,u.name FROM course_students cs JOIN users u ON u.id=cs.student_id WHERE cs.course_id=?').all(assignment.course_id))if(!ids.has(m.id))rows.push({...m,status:'not_assigned'});
  }
  const workbook = new ExcelJS.Workbook(); const sheet = workbook.addWorksheet('成绩表');
  sheet.columns = [
    { header:'小组',key:'group_name',width:18 },{ header:'学号',key:'username',width:18 },{ header:'姓名',key:'name',width:14 },{ header:'提交状态',key:'status',width:14 },
  ...
  const statusMap = { submitted:'已提交',graded:'已评分',returned:'已退回',not_assigned:'未安排' };
