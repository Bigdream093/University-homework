...
export function createFailureLimiter({ maxFailures, baseDelayMs = 1000, lockMs = 15 * 60 * 1000, windowMs = 15 * 60 * 1000 }) {
  ...
  return {
    ...
    fail(key, at = Date.now()) {
      const state = stateFor(String(key), at);
      state.failures += 1;
      const delay = state.failures >= maxFailures ? lockMs : Math.min(30_000, baseDelayMs * (2 ** Math.max(0, state.failures - 1)));
      state.blockedUntil = Math.max(state.blockedUntil, at + delay);
      return Math.max(1, Math.ceil(delay / 1000));
    },
    success(key) { states.delete(String(key)); }
  };
}
