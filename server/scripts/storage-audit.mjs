...
if (action === '--quarantine') console.log(JSON.stringify(quarantineOrphans()))
  ...
  for (const row of db.prepare('SELECT * FROM storage_quarantine WHERE deleted_at IS NULL').all()) {
    ...
    if (referencedFiles().has(target) || referencedFiles().has(path.resolve(row.original_path)))
      continue
  ...
  console.log(
    JSON.stringify({
      ...
      pending_cleanup: db
        .prepare("SELECT count(*) n FROM file_cleanup_jobs WHERE state='pending'")
        .get().n,
      quarantined_files: db
        .prepare('SELECT count(*) n FROM storage_quarantine WHERE deleted_at IS NULL')
        .get().n,
    }),
  )
