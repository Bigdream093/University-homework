...
test('submissions list exposes every append-mode file and each history file is downloadable', async () => {
  ...
  await request(app).post(`/api/courses/${course.body.id}/students`).set('Authorization', `Bearer ${teacherToken}`)
  ...
  const assignment = await request(app).post(`/api/courses/${course.body.id}/assignments`).set('Authorization', `Bearer ${teacherToken}`)
    .send({ title: '多文件作业', type: 'document', total_score: 100, allow_resubmit_count: -1, submission_mode: 'append', status: 'published' });
  ...
  await request(app).post(`/api/assignments/${assignmentId}/submit`).set('Authorization', `Bearer ${studentToken}`)
    .attach('file', Buffer.from('second version'), { filename: '补充二.zip', contentType: 'application/zip' });
  await new Promise(resolve => setTimeout(resolve, 1100));
  await request(app).post(`/api/assignments/${assignmentId}/submit`).set('Authorization', `Bearer ${studentToken}`)
    .attach('file', Buffer.from('third version'), { filename: '补充三.zip', contentType: 'application/zip' });
  ...
  assert.ok(row.files.every(f => f.history_id), '每个文件都有 history_id');
  assert.ok(row.files.every(f => f.file_name.includes('_准时.zip')), '文件名为规范命名');

  const firstHistoryId = row.files[0].history_id;
  const firstDownload = await getBinary(`/api/submissions/${submissionId}/file?history_id=${firstHistoryId}`, teacherToken);
  ...
  const invalidDownload = await getBinary(`/api/submissions/${submissionId}/file?history_id=999999`, teacherToken);
...
test('online-content submissions are listed in files and download as txt', async () => {
  ...
  await request(app).post(`/api/courses/${course.body.id}/students`).set('Authorization', `Bearer ${teacherToken}`)
    .send({ username: '20260001', name: '演示学生' });
  const assignment = await request(app).post(`/api/courses/${course.body.id}/assignments`).set('Authorization', `Bearer ${teacherToken}`)
    .send({ title: '在线作业', type: 'online', total_score: 100, allow_resubmit_count: -1, submission_mode: 'append', status: 'published' });
  assert.equal(assignment.status, 201);
  const assignmentId = assignment.body.id;
  ...
  const online = await request(app).post(`/api/assignments/${assignmentId}/submit`).set('Authorization', `Bearer ${studentToken}`)
  ...
  const download = await getBinary(`/api/submissions/${submissionId}/file?history_id=${row.files[0].history_id}`, teacherToken);
  ...
  const latestDownload = await getBinary(`/api/submissions/${submissionId}/file`, teacherToken);
  assert.equal(latestDownload.status, 200);
  assert.equal(latestDownload.body.toString(), '这是我的在线作答第一版', '不带 history_id 也返回在线内容');
});
