...
test('legacy database migration preserves rows, timestamps and files; second boot is stable',()=>{
 const dir=fs.mkdtempSync(path.join(os.tmpdir(),'mohen-migration-')),file=path.join(dir,'homework.sqlite'),uploads=path.join(dir,'uploads');
 fs.mkdirSync(uploads);const attachment=path.join(uploads,'legacy.txt');fs.writeFileSync(attachment,'historic submission');
 ...
 `);
 legacy.prepare("INSERT INTO submissions(id,assignment_id,student_id,file_url,file_name,submit_count,submitted_at) VALUES(100,100,101,?,'旧文件.txt',2,'2020-01-02 12:00:00')").run(attachment);
 legacy.prepare("INSERT INTO submission_history(id,submission_id,file_url,file_name,submitted_at) VALUES(100,100,?,'旧文件.txt','2020-01-02 12:00:00')").run(attachment);
 legacy.exec("INSERT INTO submission_history(id,submission_id,file_url,file_name,submitted_at) VALUES(101,100,NULL,'已丢失引用.txt','2020-01-01 12:00:00')");
 legacy.close();
 const code="const {db}=await import('./src/db.js');console.log(JSON.stringify({course:db.prepare('SELECT id,created_at,status FROM courses WHERE id=100').get(),a:db.prepare('SELECT work_mode,max_file_mb FROM assignments WHERE id=100').get(),history:db.prepare('SELECT id,file_url,file_state,submitted_at FROM submission_history ORDER BY id').all(),migration:db.prepare('SELECT * FROM schema_migrations').all(),foreign:db.pragma('foreign_key_check')}));db.close();";
 const run=()=>JSON.parse(execFileSync(process.execPath,['--input-type=module','-e',code],{cwd:path.resolve(import.meta.dirname,'..'),env:{...process.env,NODE_ENV:'test',TZ:'UTC',DATA_DIR:dir,UPLOAD_DIR:uploads},encoding:'utf8'}).trim());
 try{
 const first=run(),second=run();assert.deepEqual(second,first);assert.equal(first.course.created_at,'2020-01-01 12:00:00');assert.equal(first.course.status,'active');assert.equal(first.a.work_mode,'individual');assert.equal(first.a.max_file_mb,200);assert.equal(first.history.length,2);assert.equal(first.history[1].file_state,'legacy_unknown');assert.deepEqual(first.foreign,[]);assert.equal(fs.readFileSync(attachment,'utf8'),'historic submission');
...
test('front-end and back-end agree at deadline second boundaries',async()=>{
 ...
 const time=Date.parse('2026-08-31T12:00:00+08:00'),deadline='2026-08-31 12:00:00';
 assert.notEqual(deadlineState(deadline,time).kind,'late');assert.notEqual(deadlineState(deadline,time+999).kind,'late');assert.equal(deadlineState(deadline,time+1000).kind,'late');
});

test('production refuses a missing timezone before creating or migrating the database',()=>{
 const dir=fs.mkdtempSync(path.join(os.tmpdir(),'mohen-timezone-')),target=path.join(dir,'data');
